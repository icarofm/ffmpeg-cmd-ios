...
int ffmpeg(int argc, char **argv, void (*_mlog)(void*, int, const char*, va_list) )
{
    int i, ret;
    int64_t ti;
...
//     }
    ...
    if ((decode_error_stat[0] + decode_error_stat[1]) * max_error_rate < decode_error_stat[1])
        return exit2_program(69)*-1;

    return exit2_program(received_nb_signals ? 255 : main_return_code);
